...
function getCommonStyle(config = {}) {
  ...
  return `display:inline-block;will-change:transform;perspective:500px;padding:${
    ...
    conf.brgPadding * 2}px;font-weight:normal;white-space:nowarp;color:${
    conf.color
...
function getChannel(type = "normal") {
  ...
  const channelMap = {};
  if (type === "normal") {
    const normalBrgs = Brgs.filter(
      brg => brg.show && brg.sendType === "normal"
    );
    for (let brg of normalBrgs) {
      channelMap[brg.channel] = channelMap[brg.channel] || {
        num: 0,
        remain: 0
      };
      channelMap[brg.channel].num += 1;
      // 右侧未离开距离 + 20间距
    ...
    let totalChannels = Array.from(Array(brgChannel), (v, i) => i);
    let validChannels = [];
    switch (renderArea) {
      case "full":
    ...
    for (let c of alreadyChannels) {
      ...
      const cha = channelMap[c];
      if (cha.remain || cha.num >= brgChannel) {
        cacheChannels.splice(cacheChannels.indexOf(+c), 1);
      }
    }
    /**
    ...
    const fixedBrgs = Brgs.filter(brg => brg.show && brg.sendType !== "normal");
  }
  return channel;
}
